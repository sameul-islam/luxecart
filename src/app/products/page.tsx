...
export default function Page() {
  ...
  useEffect(() => {
    ...
    setCategoryFromQuery(params.get("category"))
  }, [])
  ...
  const filtered = useMemo(() => {
    ...
    if (selectedCategory) arr = arr.filter((p) => p.category === selectedCategory)
    if (selectedSizes.size > 0) arr = arr.filter((p) => p.sizes.some((s) => selectedSizes.has(s)))
    if (selectedColors.size > 0) arr = arr.filter((p) => p.colors.some((c) => selectedColors.has(c)))
